...
struct HTTP2HeadersStateMachine
{
    ...
    mutating func newHeaders(block: HTTPHeaders) -> HeaderType {
        ...
        case (.client, .none):
            ...
            newType = .requestHead
        case (.server, .none),
             (.server, .informationalResponseHead?):
...
final class HTTP2Stream
{
    ...
    /// The HPACK header encoder for this stream.
    let encoder: HpackEncoder
    
    /// The HPACK header decoder for this stream.
