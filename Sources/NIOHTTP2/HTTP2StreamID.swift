...
public class HTTP2StreamID
{
    ...
    /// A masking value used to extract the exclusive-dependency bit from a dependency
    /// stream ID.
    internal static let streamDependencyMask: Int32 = Int32(bitPattern: 0x8000_0000)
    ...
    public var networkStreamID: Int32? {
        return (actualStreamID >= 0) ? self.actualStreamID : nil
    }
    
    /// Resolve the abstract stream ID to a real one.
    internal func resolve(to newID: Int32) {
        precondition(actualStreamID == -1)
        actualStreamID = newID
    }
}
...
extension HTTP2StreamID : CustomDebugStringConvertible
{
    public var debugDescription: String {
        var streamIDDescription = "unknown"
