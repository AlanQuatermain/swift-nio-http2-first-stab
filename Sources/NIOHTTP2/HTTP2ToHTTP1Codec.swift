...
//  CNIOAtomics
//
//  Created by Jim Dovey on 6/20/18.
//
...
public final class HTTP2ToHTTP1ClientCodec : ChannelInboundHandler, ChannelOutboundHandler
{
    ...
    public func channelRead(ctx: ChannelHandlerContext, data: NIOAny) {
        ...
        case .headers(let headers), .continuation(let headers):
        ...
        case .alternativeService, .resetStream, .priority, .windowUpdate:
            // All these frames are handled outside the multiplexer.
            return
        default:
    ...
    public func write(ctx: ChannelHandlerContext, data: NIOAny, promise: EventLoopPromise<Void>?) {
        ...
        case .body(let body):
            let frame = HTTP2Frame(streamID: self.streamID, payload: .data(body))
            ctx.write(self.wrapOutboundOut(frame), promise: promise)
        case .end(let trailers):
            let frame: HTTP2Frame
            if let trailers = trailers {
                frame = HTTP2Frame(streamID: self.streamID, flags: [.endHeaders, .endStream],
                                   payload: .headers(trailers))
            }
            else {
                let buf = IOData.byteBuffer(ctx.channel.allocator.buffer(capacity: 0))
                frame = HTTP2Frame(streamID: self.streamID, flags: .endStreamm, payload: payload)
            }
            ctx.write(self.wrapOutboundOut(frame), promise: promise)
        }
    }
...
public final class HTTP2ToHTTP1ServerCodec : ChannelInboundHandler, ChannelOutboundHandler
{
    ...
    public func channelRead(ctx: ChannelHandlerContext, data: NIOAny) {
        ...
        case .alternativeService, .resetStream, .priority, .windowUpdate:
            // handled outside the multiplexer
            return
        default:
            fatalError("unimplemented frame \(frame)")
        }
    ...
    public func write(ctx: ChannelHandlerContext, data: NIOAny, promise: EventLoopPromise<Void>?) {
        ...
        switch responsePart {
        case .head(let head):
            let frame = HTTP2Frame(streamID: self.streamID, payload: .headers(head.headers))
            ctx.write(self.wrapOutboundOut(frame), promise: promise)
        case .body(let body):
            let frame = HTTP2Frame(streamID: self.streamID, payload: .data(body))
            ctx.write(self.wrapOutboundOut(frame), promise: promise)
        case .end(let trailers):
            let frame: HTTP2Frame
            if let trailers = trailers {
                frame = HTTP2Frame(streamID: self.streamID, flags: [.endStream, .endHeaders], payload: .headers(trailers))
            }
            else {
                let buf = IOData.byteBuffer(ctx.channel.allocator.buffer(capacity: 0))
                frame = HTTP2Frame(streamID: self.streamID, flags: .endStream, payload: .data(buf))
            }
