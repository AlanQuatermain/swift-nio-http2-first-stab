...
internal extension HTTPHeaders
{
    ...
    internal func withNGHTTP2Headers<T>(allocator: ByteBufferAllocator,
        ...
        return headerBlock.headerBuffer.withUnsafeMutableReadableBytes { ptr in
            ...
            for (nameBegin, nameLen, valueBegin, valueLen) in headerBlock.headerIndices {
                nghttpNVs.append(nghttp2_nv(name: base + nameBegin, value: base + valueBegin,
                                            namelen: nameLen, valuelen: valueLen, flags: 0))
            }
...
private struct ContiguousHeaderBlock
{
    ...
    mutating func writeHeader(name: String, value: String) {
        let nameBegin = self.headerBuffer.writerIndex
        let nameLen = self.headerBuffer.write(string: name)!
        let valueBegin = self.headerBuffer.writerIndex
        let valueLen = self.headerBuffer.write(string: value)!
        
        self.headerIndices.append((nameBegin, nameLen, valueBegin, valueLen))
    }
}
...
{
    /// Create a `HTTPRequestHead` from the header block produces by nghttp2.
    init(http2HeaderBlock headers: HTTPHeaders) {
        ...
        let url = headers.popPseudoHeader(name: ":path")!
        ...
        self.init(version: version, method: method, uri: url)
