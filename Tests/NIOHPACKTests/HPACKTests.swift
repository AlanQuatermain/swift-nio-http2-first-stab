...
class HPACKTests: XCTestCase {
    ...
    private func encodeInteger(_ value: UInt, prefix: Int) -> [UInt8] {
        var data = Array<UInt8>(repeating: 0, count: 11)
        let len = data.withUnsafeMutableBytes { ptr -> Int in
            NIOHPACK.encodeInteger(value, to: ptr.baseAddress!, prefix: prefix)
        }
    ...
    private func decodeInteger(from bytes: [UInt8], prefix: Int) throws -> UInt {
        return try bytes.withUnsafeBufferPointer { (buf: UnsafeBufferPointer<UInt8>) -> UInt in
            let r: UInt
            (r, _) = try NIOHPACK.decodeInteger(from: buf, prefix: prefix)
            return r
        }
    }
    ...
    private func huffmanDecode(data: [UInt8], using decoder: HuffmanDecoder) throws -> String {
        return try data.withUnsafeBytes { ptr -> String in
            try decoder.decodeString(from: ptr.baseAddress!, count: ptr.count)
        }
    }
    ...
    func verifyHuffmanCoding(_ string: String, _ bytes: [UInt8], file: StaticString = #file, line: UInt = #line) {
        let encoder = HuffmanEncoder()
        let decoder = HuffmanDecoder()
        
        XCTAssertEqual(encoder.encode(string), bytes.count, "Wrong length encoding '\(string)'", file: file, line: line)
        XCTAssertEqual(encoder.data, bytes, "Failed to encode '\(string)'", file: file, line: line)
        XCTAssertEqual(try huffmanDecode(data: bytes, using: decoder), string, "Failed to decode '\(string)'", file: file, line: line)
    }
    ...
    private func buffer<S : ContiguousCollection>(wrapping bytes: S) -> ByteBuffer where S.Element == UInt8 {
    ...
    func testComplexHuffmanEncoding() {
        ...
        let text = "鯖審"
        var encoded = buffer(wrapping: [0xff, 0xff, 0xaf, 0xff, 0xff, 0x67, 0xff, 0xfe, 0x2f, 0xff, 0xf3, 0xff, 0xff, 0xec, 0xff, 0xff, 0x77])
...
//        print("Encoded length: \(amount)")
        XCTAssertEqual(amount, encoded.readableBytes)
        let data = encoder.data
//        print("Encoded data: \(byteString(from: data))")
        XCTAssertEqual(buffer(wrapping: data), encoded)
...
//        print("Decoded utf-8 data: \(byteString(from: decoded.utf8))")
        ...
        let encoded1 = buffer(wrapping: Array(encoded1Data))
        
        let amount1 = encoder.encode(text1)
        XCTAssertEqual(amount1, encoded1.readableBytes)
        let data1 = encoder.data
//        print("\n\n\(data1.base64EncodedString())\n\n")
        XCTAssertEqual(buffer(wrapping: data1), encoded1)
        
        let decoded1 = data1.withUnsafeBytes { ptr -> String in
            guard let result = try? decoder.decodeString(from: ptr.baseAddress!, count: ptr.count) else {
                XCTFail()
                return ""
            }
            return result
        ...
        let encoded2 = buffer(wrapping: Array(encoded2Data))

        let amount2 = encoder.encode(text2)
        XCTAssertEqual(amount2, encoded2.readableBytes)
        let data2 = encoder.data
        XCTAssertEqual(buffer(wrapping: data2), encoded2)

//        print("\n\n\n\(data2.base64EncodedString())\n\n\n")

        let decoded2 = data2.withUnsafeBytes { ptr -> String in
            guard let result = try? decoder.decodeString(from: ptr.baseAddress!, count: ptr.count) else {
                XCTFail()
                return ""
            }
